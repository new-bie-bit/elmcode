...
public class UserController {
    ...
    public ResponseResult<User> getUserByIdByPass(User user) throws Exception {
        return userService.getUserByIdByPass(user);
    }

    @RequestMapping("/getUserById")
    @CircuitBreaker(name = "getUserById", fallbackMethod = "fallback")
    public ResponseResult<Integer> getUserById(User user) throws Exception {
        return userService.getUserById(user.getUserId());
    }

    @RequestMapping("/saveUser")
    @CircuitBreaker(name = "saveUser", fallbackMethod = "fallback")
    public ResponseResult<Integer> saveUser(User user) throws Exception {
